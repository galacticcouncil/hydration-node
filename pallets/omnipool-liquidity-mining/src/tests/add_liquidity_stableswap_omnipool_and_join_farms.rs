...
fn add_liquidity_stableswap_omnipool_and_join_farms_should_fail_when_origin_is_none() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			//Act and assert
			assert_noop!(OmnipoolMining::add_liquidity_stableswap_omnipool_and_join_farms(
				RuntimeOrigin::none(),
				STABLESWAP_POOL_ID,
				vec![AssetAmount::new(USDT, amount)].try_into().unwrap(),
				yield_farms.try_into().unwrap(),
...
#[test]
fn add_liquidity_stableswap_omnipool_and_join_farms_should_fail_when_no_farm_specified() {
	let token_amount = 2000 * ONE;
	ExtBuilder::default()
		.with_endowed_accounts(vec![
			(Omnipool::protocol_account(), DAI, 1000 * ONE),
