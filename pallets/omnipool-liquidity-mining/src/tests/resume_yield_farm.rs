...
fn resume_yield_farm_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let yield_farm_id = 2;
			let asset = KSM;
			let new_multiplier = FixedU128::from_float(1.234_332_f64);
...
#[test]
fn resume_yield_farm_should_fail_when_omnipool_doesnt_exists() {
	ExtBuilder::default()
		.with_endowed_accounts(vec![
			(Omnipool::protocol_account(), DAI, 1000 * ONE),
			(Omnipool::protocol_account(), HDX, NATIVE_AMOUNT),
		...
		.execute_with(|| {
			...
			assert_ok!(Omnipool::remove_asset(asset));
...
#[test]
fn resume_yield_farm_should_fail_when_origin_is_none() {
	ExtBuilder::default()
		.with_endowed_accounts(vec![
			(Omnipool::protocol_account(), DAI, 1000 * ONE),
			(Omnipool::protocol_account(), HDX, NATIVE_AMOUNT),
