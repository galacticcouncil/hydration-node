...
fn terminate_yield_farm_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			//Act & assert
			assert_ok!(OmnipoolMining::terminate_yield_farm(
				RuntimeOrigin::signed(GC),
				global_farm_id,
				yield_farm_id,
				asset
...
#[test]
fn terminate_yield_farm_should_fail_when_origin_is_none() {
	ExtBuilder::default()
		.with_endowed_accounts(vec![
			(Omnipool::protocol_account(), DAI, 1000 * ONE),
			(Omnipool::protocol_account(), HDX, NATIVE_AMOUNT),
		...
		.execute_with(|| {
			...
			//Act & assert
			assert_noop!(
				OmnipoolMining::terminate_yield_farm(RuntimeOrigin::none(), global_farm_id, yield_farm_id, asset),
				BadOrigin
			);
		});
}
