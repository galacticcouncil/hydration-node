...
use sp_runtime::{
	traits::{
		Convert, DispatchInfoOf, Dispatchable, One, PostDispatchInfoOf, SaturatedConversion, Saturating,
	...
	X: Get<Multiplier>,
{
	fn convert(previous: Multiplier) -> Multiplier {
		...
		log::info!("---CONVERT INFO---");
		log::info!(
			"min_multiplier {:?}, max_multiplier: {:?}, previous: {:?}",
			min_multiplier.into_inner(),
			max_multiplier.into_inner(),
			previous.into_inner()
		);
...
pub mod pallet {
	...
	pub fn NextFeeMultiplierOnEmpty() -> Multiplier {
		Multiplier::saturating_from_rational(1, 1_000_000u128)
	}

	#[pallet::storage]
	#[pallet::getter(fn next_fee_multiplier)]
	pub type NextFeeMultiplier<T: Config> = StorageValue<_, Multiplier, ValueQuery, NextFeeMultiplierOnEmpty>;
	...
	impl Default for GenesisConfig {
		fn default() -> Self {
			let multiplier = Multiplier::saturating_from_rational(1, 1_000_000u128);
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn on_finalize(_: T::BlockNumber) {
			<NextFeeMultiplier<T>>::mutate(|fm| {
				log::info!("ON FINALIZE:  previous multiplier: {:?}", fm.into_inner());
	...
	BalanceOf<T>: FixedPointOperand,
{
	...
	{
		log::info!("---- QUERY CALL INFO ----");
		log::info!("CALL {:?}", call);
		log::info!("CALL encoded {:?}", call.encode());
		log::info!("LEN {:?}", len);
	...
	where
		T::RuntimeCall: Dispatchable<Info = DispatchInfo> + GetDispatchInfo,
	{
		log::info!("---- QUERY CALL FEE DETAILS ----");
		log::info!("CALL {:?}", call);
		log::info!("CALL encoded {:?}", call.encode());
		log::info!("LEN {:?}", len);
		let dispatch_info = <T::RuntimeCall as GetDispatchInfo>::get_dispatch_info(&call);
	...
	where
		T::RuntimeCall: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
	{
		log::info!("----COMPUTE ACTUAL FEE DETAILS----");
...
impl<T: Config, AnyCall: GetDispatchInfo + Encode> EstimateCallFee<AnyCall, BalanceOf<T>> for Pallet<T>
where
	BalanceOf<T>: FixedPointOperand,
	T::RuntimeCall: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
