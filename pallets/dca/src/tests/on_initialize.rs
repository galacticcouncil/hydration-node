		...
		.execute_with(|| {
			...
			assert_executed_sell_trades!(vec![
				...
				SellExecution {
					asset_in: HDX,
					asset_out: BTC,
					amount_in: total_amount - 2 * (amount_to_sell + SELL_DCA_FEE_IN_NATIVE) - SELL_DCA_FEE_IN_NATIVE,
					min_buy_amount: *AMOUNT_OUT_FOR_OMNIPOOL_SELL,
				}
			]);
...
fn sell_schedule_should_continue_when_there_is_exact_amount_in_left_as_remaining() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount = 3 * *AMOUNT_OUT_FOR_OMNIPOOL_SELL * 2 + SELL_DCA_FEE_IN_NATIVE;
			let amount_to_sell = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
...
fn full_sell_dca_should_be_completed_when_some_successful_dca_execution_happened_but_less_than_fee_left() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let amount_to_sell = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
			let total_amount = 2 * (amount_to_sell + SELL_DCA_FEE_IN_NATIVE) + SELL_DCA_FEE_IN_NATIVE / 2;
...
fn full_buy_dca_should_be_completed_when_some_successful_dca_execution_happened_but_less_than_fee_left() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount =
				2 * (CALCULATED_AMOUNT_IN_FOR_OMNIPOOL_BUY + BUY_DCA_FEE_IN_NATIVE) + BUY_DCA_FEE_IN_NATIVE / 2;
			let amount_to_buy = 10 * ONE;
...
fn sell_dca_should_be_completed_when_last_trade_amount_is_total_budget_plus_fee() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let amount_to_sell = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
			let total_amount = 2 * (amount_to_sell + SELL_DCA_FEE_IN_NATIVE);
...
fn dca_sell_schedule_should_be_terminated_when_schedule_allocation_is_more_than_reserved_funds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let amount_in = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
			let total_amount = 2 * (amount_in + SELL_DCA_FEE_IN_NATIVE);
			let schedule = ScheduleBuilder::new()
				.with_period(ONE_HUNDRED_BLOCKS)
				.with_total_amount(total_amount)
				.with_order(Order::Sell {
...
fn sell_schedule_should_be_completed_when_remainder_is_less_than_20_transaction_fee() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount = 2 * (ONE + SELL_DCA_FEE_IN_NATIVE) + remainder;
			let amount_to_sell = ONE;
...
fn dca_should_continue_when_remainder_is_equal_to_min_trading_limit() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount = 2 * (ONE + SELL_DCA_FEE_IN_NATIVE) + min_trade_limit;
			let amount_to_sell = ONE;
