...
fn successful_buy_dca_execution_should_emit_trade_executed_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			insert_schedule_into_storage(ALICE, schedule, Option::None);
...
fn sell_schedule_should_sell_remaining_when_there_is_not_enough_left() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount = *AMOUNT_OUT_FOR_OMNIPOOL_SELL * 5 / 2;
			let amount_to_sell = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
...
fn sell_schedule_should_continue_when_there_is_exact_amount_in_left_as_remaining() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount = 3 * *AMOUNT_OUT_FOR_OMNIPOOL_SELL * 2 + sell_dca_fee;
			let amount_to_sell = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
...
fn one_buy_dca_execution_should_unreserve_exact_amount_in() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			insert_schedule_into_storage(ALICE, schedule.clone(), Option::None);
			assert_eq!(total_amount, Currencies::reserved_balance(HDX, &ALICE));
...
fn full_sell_dca_should_be_completed_with_selling_leftover_in_last_trade() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			//Act
			proceed_to_blocknumber(502, 802);
...
fn full_sell_dca_should_be_completed_when_some_successful_dca_execution_happened_but_less_than_fee_left() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let sell_dca_fee = get_fee_for_sell_in_hdx();
			let amount_to_sell = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
			let total_amount = 2 * (amount_to_sell + sell_dca_fee) + sell_dca_fee / 2;
			...
			assert_number_of_executed_sell_trades!(2);
			assert_balance!(ALICE, BTC, 2 * *AMOUNT_OUT_FOR_OMNIPOOL_SELL);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				...
				budget - amount_in - sell_fee_in_dai
			);
...
fn sell_dca_native_execution_fee_should_be_taken_and_sent_to_treasury() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Currencies::reserved_balance(HDX, &ALICE),
				total_amount - *AMOUNT_OUT_FOR_OMNIPOOL_SELL - sell_fee_in_native
			);
			assert_balance!(ALICE, BTC, *AMOUNT_OUT_FOR_OMNIPOOL_SELL);
			assert_number_of_executed_sell_trades!(1);
		});
...
#[test]
fn sell_dca_should_be_completed_when_last_trade_amount_is_total_budget_plus_fee() {
	ExtBuilder::default()
		.with_endowed_accounts(vec![(ALICE, HDX, 10000 * ONE)])
		.build()
		.execute_with(|| {
			...
			let amount_to_sell = *AMOUNT_OUT_FOR_OMNIPOOL_SELL;
			let total_amount = 2 * (amount_to_sell + get_fee_for_sell_in_hdx());
...
fn buy_dca_native_execution_fee_should_be_taken_and_sent_to_treasury() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Currencies::reserved_balance(HDX, &ALICE),
				budget - CALCULATED_AMOUNT_IN_FOR_OMNIPOOL_BUY - buy_fee_in_native
			);
...
fn one_sell_dca_execution_should_be_rescheduled_when_price_diff_is_more_than_user_specified_treshold() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_executed_sell_trades!(vec![]);
			assert_eq!(
				total_amount - get_fee_for_sell_in_hdx(),
				Currencies::reserved_balance(HDX, &ALICE)
			);
...
fn dca_should_be_terminated_when_dca_cannot_be_planned_due_to_not_free_blocks() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(DCA::schedule(RuntimeOrigin::signed(ALICE), schedule, Option::Some(995)));
			assert_eq!(total_amount, Currencies::reserved_balance(HDX, &ALICE));
...
fn dca_sell_schedule_should_be_terminated_when_schedule_allocation_is_more_than_reserved_funds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			Currencies::unreserve_named(&NamedReserveId::get(), HDX, &ALICE, 5000 * ONE);
...
fn sell_schedule_should_be_completed_when_remainder_is_less_than_20_transaction_fee() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount = 2 * (ONE + sell_fee) + remainder;
			let amount_to_sell = ONE;
...
fn schedules_are_purged_when_the_block_is_over() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			set_to_blocknumber(502);
			assert_number_of_executed_sell_trades!(3);
			set_to_blocknumber(503);

			//Assert
			let scheduled_ids_for_next_block = DCA::schedule_ids_per_block(502);
			assert_eq!(
...
fn sell_schedule_should_be_replanned_when_remainder_is_equal_to_20_transaction_fee() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let remainder = 20 * get_fee_for_sell_in_hdx();
			let total_amount = 2 * (ONE + get_fee_for_sell_in_hdx()) + remainder;
			let amount_to_sell = ONE;
...
fn sell_schedule_should_be_replanned_when_more_than_20_transaction_fee_left_for_next_trade() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let remainder = 20 * get_fee_for_sell_in_hdx() + 1;
			let total_amount = 2 * (ONE + get_fee_for_sell_in_hdx()) + remainder;
			let amount_to_sell = ONE;
...
fn dca_should_complete_when_remainder_is_smaller_than_min_trading_limit() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_amount = 2 * (ONE + fee) + remainder;
			let amount_to_sell = ONE;
...
fn schedule_is_planned_for_next_block_when_one_execution_finished() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let schedule_id = 0;
			assert_scheduled_ids!(602, vec![schedule_id]);
		});
}
...
fn dca_trade_unallocation_should_be_rolled_back_when_trade_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let fee_in_native = DCA::get_transaction_fee(&schedule.order, None).unwrap();
			assert_eq!(Currencies::reserved_balance(HDX, &ALICE), total_amount - fee_in_native);
			assert_eq!(
...
fn execution_fee_should_be_taken_from_user_in_sold_currency_in_case_of_successful_buy_trade() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Currencies::reserved_balance(DAI, &ALICE),
				budget - CALCULATED_AMOUNT_IN_FOR_OMNIPOOL_BUY - buy_fee_in_dai
			);
			assert_balance!(ALICE, BTC, CALCULATED_AMOUNT_IN_FOR_OMNIPOOL_BUY);
		});
}
...
fn specified_slippage_should_be_used_in_circuit_breaker_price_check() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// Short oracle prices the sell of ONE at 0.8 * ONE, so the limit is 0.728 * ONE at the
			// schedule's 9% and 0.72 * ONE at the pallet default. 0.725 * ONE sits between the two:
			// it only fails if the specified slippage is the one being applied.
			set_sell_amount_out(ONE * 725 / 1000);
			let schedule = ScheduleBuilder::new()
				.with_total_amount(total_amount)
				.with_period(ONE_HUNDRED_BLOCKS)
...
fn dca_schedule_should_still_take_fee_when_order_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			set_to_blocknumber(502);
			assert_number_of_executed_sell_trades!(0);
			assert_balance!(TreasuryAccount::get(), HDX, fee_in_native);
		});
}
