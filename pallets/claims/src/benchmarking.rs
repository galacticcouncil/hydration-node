...
benchmarks! {
	...
	// Benchmark for validate_claim (used by TransactionExtension)
	// Measures ECDSA signature recovery + 1 storage read
	validate_claim {
		let alice_id = hex!["d43593c715fdd31c61141abd04a99fd6822c8558854ccde39a5684e7a56da27d"];
		let signature = hex!["bcae7d4f96f71cf974c173ae936a1a79083af7f76232efbf8a568b7f990eceed73c2465bba769de959b7f6ac5690162b61eb90949901464d0fa158a83022a0741c"];
		...
		let _ = Pallet::<T>::validate_claim(&caller, &EcdsaSignature(signature));
	}
	verify {
		// Claim should still exist (validate doesn't consume it)
		assert_eq!(Claims::<T>::get(eth_address), T::CurrencyBalance::from(amount_to_claim).into());
	}
}
...
mod tests {
	...
	fn test_benchmarks() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Pallet::<Test>::test_benchmark_validate_claim());
		});
	}
}
