...
#[test]
fn remove_liquidity_should_work_when_withdrawing_min_trade_limit() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let shares = MinimumTradingLimit::get();
			...
			let withdrawn_liquidity = Tokens::free_balance(asset_c, &BOB);
			assert!(
				withdrawn_liquidity > 0,
				"There should be some wihdrawn liquidity, but there is none"
			);
		});
}
