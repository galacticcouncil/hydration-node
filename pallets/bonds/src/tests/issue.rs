...
fn issue_bonds_should_work_when_fee_is_zero() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(hdx_asset_details.existential_deposit, bond_asset_details.existential_deposit);
...
fn issue_bonds_should_work_when_issuing_multiple_bonds() {
	ExtBuilder::default()
		.with_protocol_fee(Permill::from_percent(10))
		.with_registered_asset(DAI, AssetDetailsT {
		...
		assert_eq!(hdx_asset_details.existential_deposit, first_bond_asset_details.existential_deposit);
		assert_eq!(hdx_asset_details.existential_deposit, second_bond_asset_details.existential_deposit);
		assert_eq!(dai_asset_details.existential_deposit, third_bond_asset_details.existential_deposit);

		assert_eq!(Tokens::free_balance(HDX, &ALICE), INITIAL_BALANCE - amount.checked_mul(2).unwrap());
		...
		assert_eq!(Tokens::free_balance(HDX, &<Test as Config>::FeeReceiver::get()), fee.checked_mul(2).unwrap());
		assert_eq!(Tokens::free_balance(DAI, &<Test as Config>::FeeReceiver::get()), fee);

		assert_eq!(Tokens::free_balance(HDX, &Bonds::account_id()), amount_without_fee.checked_mul(2).unwrap());
