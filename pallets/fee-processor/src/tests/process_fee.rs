...
fn process_trade_fee_same_asset_twice_does_not_duplicate_pending_count() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		// Pot accumulates both fees (mock receivers sum to 100% so take == amount).
		let pot = FeeProcessor::pot_account_id();
		let pot_dot = <FungibleCurrencies<Test> as Inspect<AccountId>>::balance(DOT, &pot);
		assert_eq!(pot_dot, 300 * ONE);
	});
}
...
fn process_trade_fee_nothing_changes_when_pre_deposit_callback_fails() {
	...
	// when a pre-deposit callback errors, the asset transfer to the pot AND the
	// PendingConversions insert are both rolled back — no funds get stuck.
	use frame_support::storage::with_transaction;
	use sp_runtime::TransactionOutcome;

	ExtBuilder::default().build().execute_with(|| {
		let pot = FeeProcessor::pot_account_id();
		let source_dot_before = <FungibleCurrencies<Test> as Inspect<AccountId>>::balance(DOT, &FEE_SOURCE);
		let pot_dot_before = <FungibleCurrencies<Test> as Inspect<AccountId>>::balance(DOT, &pot);

		set_pre_deposit_should_fail(true);
		...
		// Balances unchanged
		let source_dot_after = <FungibleCurrencies<Test> as Inspect<AccountId>>::balance(DOT, &FEE_SOURCE);
		let pot_dot_after = <FungibleCurrencies<Test> as Inspect<AccountId>>::balance(DOT, &pot);
		assert_eq!(source_dot_after, source_dot_before, "source DOT must not have moved");
		assert_eq!(pot_dot_after, pot_dot_before, "pot must not have received DOT");
