...
fn deferred_by_should_track_incoming_deposited_asset_liquidity() {
	ExtBuilder::default().build().execute_with(|| {
		...
		//Assert
		let volume = XcmRateLimiter::liquidity_per_asset(MultiLocation::here());
		assert_eq!(volume, 5);
	});
}
*/
...
fn deferred_by_should_defer_successive_xcm_when_time_passes() {
	ExtBuilder::default().build().execute_with(|| {
		...
		//Assert
		let (accumulated_amount, last_update) = XcmRateLimiter::liquidity_per_asset(MultiLocation::here());
