...
fn new_liquidity(entry: &OracleEntry) -> Balance {
	entry.liquidity + entry.amount_in - entry.amount_out
}
...
proptest! {
	...
	fn fees_should_update_correctly_when_volume_out_increasing(previous_fee in initial_fee(),
		...
		let current_liquidity = new_liquidity(&entry);
		...
		let asset_fee = recalculate_asset_fee(entry.clone(), current_liquidity, previous_fee, block_diff, params.into());
		let protocol_fee = recalculate_protocol_fee(entry, current_liquidity, previous_fee, block_diff, params.into());
