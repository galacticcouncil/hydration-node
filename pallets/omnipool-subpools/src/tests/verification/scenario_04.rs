...
fn subpool_trades_should_work_correct_when_trade_asset_in_given_stable_out() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(usda_alice, 9499371234838227);
			assert_eq!(r1_alice, amount_to_buy);

			assert_eq!(
				r1_state,
				AssetReserveState {
					reserve: 999000000000000000,
					hub_reserve: 50050050050050050051,
			...
			assert_eq!(
				...
				vec![1000500628765161773, 1000000000000000000]
			);

			assert_eq!(
				subpool_share_state,
				AssetReserveState {
					reserve: 200050062578222778475,
					hub_reserve: 199949949949949949949,
