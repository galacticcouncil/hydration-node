...
#[test]
fn note_egress_should_trigger_lockdown_when_limit_exceeded() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(CircuitBreaker::set_global_withdraw_limit(RuntimeOrigin::root(), 1000));
		
		assert_ok!(CircuitBreaker::note_egress(999));
		// note_egress returns error when limit exceeded, but still updates storage
		...
		let until = CircuitBreaker::withdraw_lockdown_until().unwrap();
		assert!(until > 0);
	});
}

#[test]
fn note_egress_should_fail_during_lockdown() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(CircuitBreaker::set_global_withdraw_limit(RuntimeOrigin::root(), 1000));
		let _ = CircuitBreaker::note_egress(1001);
...
fn set_global_withdraw_limit_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		expect_events(vec![
			Event::GlobalLimitUpdated { new_limit: 1000 }.into(),
		]);
	});
}
