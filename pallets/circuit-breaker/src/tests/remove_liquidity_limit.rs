...
fn liquidity_limit_should_be_stored_when_called_first_time() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(CircuitBreaker::allowed_remove_liquidity_limit_per_asset(HDX), None);
...
fn liquidity_limit_should_not_be_stored_for_omnipool_hub_asset() {
	ExtBuilder::default().build().execute_with(|| {
		// Arrange
		// storage should be empty at the beginning
		assert_eq!(CircuitBreaker::allowed_remove_liquidity_limit_per_asset(LRNA), None);
...
fn ensure_and_update_liquidity_limit_should_work_when_liquidity_is_within_limit() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// Act & Assert
		assert_ok!(CircuitBreaker::ensure_and_update_remove_liquidity_limit(HDX, 200_000));
	});
}
...
fn ensure_and_update_liquidity_limit_should_not_throw_error_when_turned_off() {
	ExtBuilder::default().build().execute_with(|| {
		//Arrange
		assert_ok!(CircuitBreaker::set_remove_liquidity_limit(RuntimeOrigin::root(), HDX, None,));
		...
		assert!(CircuitBreaker::allowed_remove_liquidity_limit_per_asset(HDX).is_none());

		// Act & Assert
		assert_ok!(CircuitBreaker::ensure_and_update_remove_liquidity_limit(HDX, 1_000_000));
	});
}
...
fn ensure_and_update_liquidity_limit_should_not_throw_error_when_turned_off_after_storing_limit() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			CircuitBreaker::ensure_and_update_remove_liquidity_limit(HDX, 1_000_000),
...
fn ensure_and_update_liquidity_limit_should_fail_when_max_limit_is_reached() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// Act & Assert
		assert_noop!(
			CircuitBreaker::ensure_and_update_remove_liquidity_limit(HDX, 200_001),
			Error::<Test>::MaxLiquidityLimitPerBlockReached
		);
	});
}
...
fn ensure_and_update_liquidity_limit_should_fail_when_liquidity_limit_not_stored() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			CircuitBreaker::ensure_and_update_remove_liquidity_limit(HDX, INITIAL_LIQUIDITY),
...
fn set_liquidity_limit_should_store_new_trade_volume_limit() {
	ExtBuilder::default().build().execute_with(|| {
		// Arrange & Act
		let default_limit = <Test as Config>::DefaultMaxRemoveLiquidityLimitPerBlock::get();
		assert_eq!(default_limit, DefaultRemoveLiquidityLimit::<Test>::get());
...
#[test]
#[should_panic(expected = "Circuit Breaker: Max remove liquidity limit per block is set to invalid value.")]
fn integrity_test_should_fail_when_liquidity_limit_numerator_is_zero() {
	use frame_support::traits::Hooks;
	ExtBuilder::default()
		.with_max_remove_liquidity_limit_per_block(Some((0, 10_000)))
		.build()
		.execute_with(|| {
...
fn integrity_test_should_fail_when_liquidity_limit_denominator_is_zero() {
	use frame_support::traits::Hooks;
	ExtBuilder::default()
		.with_max_remove_liquidity_limit_per_block(Some((2_000, 0)))
		.build()
		.execute_with(|| {
			CircuitBreaker::integrity_test();
		});
...
fn integrity_test_should_fail_when_liquidity_limit_numerator_is_too_big() {
	use frame_support::traits::Hooks;
	ExtBuilder::default()
		.with_max_remove_liquidity_limit_per_block(Some((MAX_LIMIT_VALUE.checked_add(1).unwrap(), 10_000)))
		.build()
		.execute_with(|| {
			CircuitBreaker::integrity_test();
		});
...
fn integrity_test_should_fail_when_liquidity_limit_denominator_is_too_big() {
	use frame_support::traits::Hooks;
	ExtBuilder::default()
		.with_max_remove_liquidity_limit_per_block(Some((2_000, MAX_LIMIT_VALUE.checked_add(1).unwrap())))
		.build()
		.execute_with(|| {
			CircuitBreaker::integrity_test();
		});
