...
fn test_me() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(CircuitBreaker::calculate_and_store_liquidity_limit(LRNA, INITIAL_LIQUIDITY));
		assert_ok!(CircuitBreaker::calculate_and_store_liquidity_limit(HDX, INITIAL_LIQUIDITY));
		assert_ok!(CircuitBreaker::calculate_and_store_liquidity_limit(DOT, INITIAL_LIQUIDITY));
		let _ = CircuitBreaker::on_initialize(1);
	});
}
...
#[test]
fn trade_volume_storage_should_be_cleared_at_the_beginning_of_block() {
	ExtBuilder::default().build().execute_with(|| {
		//Arrange
		assert_ok!(CircuitBreaker::calculate_and_store_trade_limit(HDX, INITIAL_LIQUIDITY));
		assert_eq!(
		...
		// Act
		CircuitBreaker::on_initialize(2);
...
#[test]
fn liquidity_storage_should_be_cleared_at_the_beginning_of_block() {
	ExtBuilder::default().build().execute_with(|| {
		//Arrange
		assert_ok!(CircuitBreaker::calculate_and_store_liquidity_limit(
			HDX,
