...
fn exit_farms_should_work_for_all_joined_farms() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			//Act
			let yield_farms = vec![(deposit_id, 3, BSX_KSM_ASSET_PAIR), (deposit_id, 4, BSX_KSM_ASSET_PAIR)];
...
fn exit_farms_should_fail_with_no_origin() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let yield_farms = vec![(deposit_id, 3, BSX_KSM_ASSET_PAIR), (deposit_id, 4, BSX_KSM_ASSET_PAIR)];
			assert_noop!(
				LiquidityMining::exit_farms(Origin::none(), yield_farms.try_into().unwrap(),),
				BadOrigin
			);
		});
}
...
fn exit_farms_should_fail_with_non_nft_owner() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let yield_farms = vec![(deposit_id, 3, BSX_KSM_ASSET_PAIR), (deposit_id, 4, BSX_KSM_ASSET_PAIR)];
			assert_noop!(
				LiquidityMining::exit_farms(Origin::signed(BOB), yield_farms.try_into().unwrap(),),
