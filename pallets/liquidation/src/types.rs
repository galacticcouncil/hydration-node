...
pub mod offchain_worker {
    ...
    use sp_std::{ops::BitAnd, boxed::Box};
    use fp_evm::{
        ExitReason::Succeed,
    ...
    impl UserData {
        pub fn new<Block, Runtime>(money_market: &MoneyMarketData<Block, Runtime>, address: H160, current_evm_timestamp: u64, caller: EvmAddress) -> Option<Self>
        where
            Block: BlockT,
            Runtime: EthereumRuntimeRPCApiV5<Block>,
        {
            let configuration = UserConfiguration(Self::get_user_configuration::<Block, Runtime>(money_market.pool_contract, address, caller)?);
            ...
            for (index, reserve) in money_market.reserves.iter().enumerate() {
                ...
                let (collateral, debt) = if configuration.is_collateral(index) || configuration.is_debt(index) {
                    let c = reserve.get_user_collateral_in_base_currency::<Block, Runtime>(address, current_evm_timestamp, caller).unwrap_or_default();
                    let d = reserve.get_user_debt_in_base_currency::<Block, Runtime>(address, current_evm_timestamp, caller).unwrap_or_default();
                    (c, d)
                } else {
                    (U256::zero(), U256::zero())
                };
            ...
            Some(Self { address, configuration, reserves, current_evm_timestamp })
        }
        ...
        pub fn get_user_configuration<Block, Runtime>(mm_pool: EvmAddress, user: EvmAddress, caller: EvmAddress) -> Option<U256>
        where
            Block: BlockT,
            Runtime: EthereumRuntimeRPCApiV5<Block>,
        {
            ...
            let call_info = Runtime::call(caller, mm_pool, data, U256::zero(), gas_limit, None, None, None, true, None).ok()?;
        ...
        Runtime: EthereumRuntimeRPCApiV5<Block>,
    {
        fn scaled_balance_of(self, user: EvmAddress, caller: EvmAddress) -> Option<U256>
        {
            ...
            let gas_limit = U256::from(500_000);
            let call_info = Runtime::call(caller, self, data, U256::zero(), gas_limit, None, None, None, true, None).ok()?;
    ...
    impl Reserve {
        ...
        pub fn get_user_collateral_in_base_currency<Block, Runtime>(&self, user: EvmAddress, current_timestamp: u64, caller: EvmAddress) -> Option<U256>
        where
            Block: BlockT,
            Runtime: EthereumRuntimeRPCApiV5<Block>,
        {
        ...
        fn get_normalized_debt(&self, current_timestamp: u64) -> Option<U256> {
            ...
            if current_timestamp == self.reserve_data.last_update_timestamp {
                ...
                let compound_interest = rate.checked_mul(exp)?.checked_div(seconds_per_year)?.checked_add(ray)?.checked_add(second_term)?.checked_add(third_term)?;
        ...
        pub fn get_user_debt_in_base_currency<Block, Runtime>(&self, user: EvmAddress, current_timestamp: u64, caller: EvmAddress) -> Option<U256>
        where
            Block: BlockT,
            Runtime: EthereumRuntimeRPCApiV5<Block>,
        {
            ...
            total_debt.checked_mul(self.price)?
                .checked_div(U256::from(10u128.pow(self.get_decimals() as u32)))
        }
    ...
    impl<Block: BlockT, Runtime: EthereumRuntimeRPCApiV5<Block>> MoneyMarketData<Block, Runtime> {
        pub fn new(pap_contract: EvmAddress, caller: EvmAddress) -> Option<Self> {
            ...
            for asset_address in Self::get_reserves_list(pool_contract, caller)?.into_iter() {
                ...
                reserves.push(Reserve { reserve_data, asset_address, symbol, price });
            };
        ...
        pub fn get_pool(pap_contract: EvmAddress, caller: EvmAddress) -> Option<EvmAddress> {
            ...
            let call_info = Runtime::call(caller, pap_contract, data, U256::zero(), gas_limit, None, None, None, true, None).ok()?;
        ...
        fn get_asset_symbol(asset_address: &EvmAddress, caller: EvmAddress) -> Option<Vec<u8>> {
            ...
            let call_info = Runtime::call(caller, *asset_address, data, U256::zero(), gas_limit, None, None, None, true, None).ok()?;
        ...
        pub fn get_asset_price(oracle_address: EvmAddress, asset: EvmAddress, caller: EvmAddress) -> Option<U256> {
            ...
            let gas_limit = U256::from(500_000);
            let call_info = Runtime::call(caller, oracle_address, data, U256::zero(), gas_limit, None, None, None, true, None).ok()?;
        ...
        ) -> Option<((U256, U256), (U256, U256))>
        {
            ...
            for (index, reserve) in self.reserves().iter().enumerate() {
                ...
                weighted_total_collateral = weighted_total_collateral
                    .checked_add(user_balances.collateral.checked_mul(U256::from(reserve.liquidation_threshold()))?)?;
            ...
            let n = total_debt.checked_mul(target_health_factor)?.checked_div(unit_price)?
                .checked_sub(weighted_total_collateral)?.checked_mul(unit_price)?;

            let d = percentage_factor.checked_mul(target_health_factor)?.checked_div(unit_price)?
                .checked_sub(liquidation_bonus.checked_mul(collateral_liquidation_threshold)?
            ...
            let base_collateral_amount = actual_debt_to_liquidate.full_mul(debt_price).checked_div(collateral_price.into())?.try_into().ok()?;
            let collateral_amount = percent_mul(base_collateral_amount, liquidation_bonus)?;

            let debt_in_base_currency = actual_debt_to_liquidate.full_mul(debt_price).checked_div(unit_price.into())?.try_into().ok()?;
            let collateral_in_base_currency = collateral_amount.full_mul(collateral_price).checked_div(unit_price.into())?.try_into().ok()?;
            log::info!("\n\n\n\n\n ------- {:?}",((actual_debt_to_liquidate, collateral_amount), (debt_in_base_currency, collateral_in_base_currency)));
            Some(((actual_debt_to_liquidate, collateral_amount), (debt_in_base_currency, collateral_in_base_currency)))
        }
        ...
        ) -> Option<Vec<LiquidationOption>>
        {
            ...
            for &(index_c, collateral_asset) in collateral_assets.iter() {
                for &(index_d, debt_asset) in debt_assets.iter() {
                    let Some(((debt_to_liquidate, _collateral_received), (debt_to_liquidate_in_base, collateral_received_in_base))) = self.calculate_debt_to_liquidate(&user_data, target_health_factor, collateral_asset, debt_asset)
                        else {
                            continue
                        };
                    ...
                    if let Some(hf) = maybe_hf {
                        liquidation_options.push(LiquidationOption::new(hf, debt_asset.clone(), collateral_asset.clone(), debt_to_liquidate));
                    }
                }
            }
    ...
    impl LiquidationOption {
        pub fn new(health_factor: U256, collateral_asset: EvmAddress, debt_asset: EvmAddress, debt_to_liquidate: U256) -> Self {
            LiquidationOption {
                health_factor,
                collateral_asset,
                debt_asset,
