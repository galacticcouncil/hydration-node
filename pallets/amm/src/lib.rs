	...
	fn validate_sell(
		...
		let discount_fee = if discount {
			...
			let native_pair_account = Self::get_pair_id(AssetPair {
				asset_in: assets.asset_in,
				asset_out: native_asset_id,
			});

			let native_asset_reserve = T::Currency::free_balance(native_asset_id, &native_pair_account);
			let asset_reserve = T::Currency::free_balance(assets.asset_in, &native_pair_account);

			let native_fee_spot_price =
				hydra_dx_math::calculate_spot_price(asset_reserve, native_asset_reserve, transfer_fee)
					.map_err(|_| Error::<T>::CannotApplyDiscount)?;

			ensure!(
				T::Currency::free_balance(native_asset_id, who) >= native_fee_spot_price,
