...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + pallet_democracy::Config + pallet_staking::Config {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn vote(
			...
			pallet_staking::Pallet::<T>::do_democracy_vote()?;
...
// The following functions are here only for test purposes to keep the original democracy tests working
#[cfg(test)]
mod test_support {
	...
	use pallet_democracy::{
		BoundedCallOf, DepositOf, PropIndex, ReferendumCount, ReferendumIndex, ReferendumInfo, ReferendumInfoOf,
		ReferendumStatus, VoteThreshold,
	};
	...
	impl<T: Config> Pallet<T> {
		pub fn lowest_unbaked() -> ReferendumIndex {
			pallet_democracy::LowestUnbaked::<T>::get()
		}
		pub fn referendum_count() -> ReferendumIndex {
			pallet_democracy::ReferendumCount::<T>::get()
		}
		pub fn referendum_info(
			index: ReferendumIndex,
		) -> Option<ReferendumInfo<T::BlockNumber, BoundedCallOf<T>, BalanceOf<T>>> {
			pallet_democracy::ReferendumInfoOf::<T>::get(index)
		}
		pub fn backing_for(proposal: PropIndex) -> Option<BalanceOf<T>> {
			pallet_democracy::Pallet::<T>::backing_for(proposal)
		}
		...
		pub fn referendum_status(
			...
			let info = ReferendumInfoOf::<T>::get(ref_index).ok_or(pallet_democracy::Error::<T>::ReferendumInvalid)?;
			match info {
				ReferendumInfo::Ongoing(s) => Ok(s),
				_ => Err(pallet_democracy::Error::<T>::ReferendumInvalid.into()),
			}
		}

		pub fn len_of_deposit_of(proposal: PropIndex) -> Option<u32> {
			Self::decode_compact_u32_at(&<DepositOf<T>>::hashed_key_for(proposal))
		}
