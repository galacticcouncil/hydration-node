...
use frame_support::{
	...
	traits::{
		...
		Bounded, Currency, Get, LockIdentifier, LockableCurrency, OnUnbalanced, QueryPreimage, ReservableCurrency,
		StorePreimage, WithdrawReasons,
	},
	weights::Weight,
};
use sp_runtime::{
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn fast_track(
			...
			Self::inject_referendum(now.saturating_add(voting_period), ext_proposal, threshold, delay);
			Ok(())
		}
...
impl<T: Config> Pallet<T> {
	...
	fn try_vote(who: &T::AccountId, ref_index: ReferendumIndex, vote: AccountVote<BalanceOf<T>>) -> DispatchResult {
		...
		T::DemocracyHooks::on_vote(who, ref_index, vote)?;
		Ok(())
	}
