...
/// tb1q38c2sg5nc3v0tkcle2uncdyqspc4kuawjxpf0j
pub const TESTNET_VAULT_ADDRESS: [u8; 20] = [
	0x89, 0xf0, 0xa8, 0x23, 0x93, 0x8c, 0x58, 0xcf, 0x5b, 0x17, 0xc8, 0xeb, 0x93, 0xc6, 0x82, 0x80, 0x63, 0x5b, 0x73,
	0x4e,
];
...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + pallet_build_btc_tx::Config + pallet_signet::Config {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn deposit_btc(
			origin: OriginFor<T>,
			request_id: [u8; 32],
			inputs: BoundedVec<pallet_build_btc_tx::UtxoInput, T::MaxInputs>,
			outputs: BoundedVec<pallet_build_btc_tx::BitcoinOutput, T::MaxOutputs>,
			lock_time: u32,
		) -> DispatchResult {
			...
			// Build PSBT
			let psbt_bytes = pallet_build_btc_tx::Pallet::<T>::build_bitcoin_tx(
			...
			let txid = pallet_build_btc_tx::Pallet::<T>::get_txid(
				frame_system::RawOrigin::Signed(requester.clone()).into(),
				inputs.clone(),
				outputs.clone(),
				lock_time,
