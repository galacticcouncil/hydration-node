...
fn test_deposit_btc_fails_without_initialization() {
    new_test_ext().execute_with(|| {
        ...
        let inputs = BoundedVec::try_from(vec![pallet_build_btc_tx::UtxoInput {
            txid: [0x42; 32],
            vout: 0,
            value: 100_000_000,
        ...
        let outputs = BoundedVec::try_from(vec![pallet_build_btc_tx::BitcoinOutput {
            value: 99_900_000,
            script_pubkey: BoundedVec::try_from(vec![0x00, 0x14]).unwrap(),
        }])
        .unwrap();
