...
fn withdraw_shares_should_send_reward_to_user_when_reward_is_less_than_ed_but_user_has_balance() {
	...
	Hydra::execute_with(|| {
		...
		let expected_claimed_amount = 184_023_908u128;
		assert_eq!(
			hydradx_runtime::Currencies::free_balance(HDX, &CHARLIE.into()),
...
fn claim_rewards_should_work_when_farm_is_updated() {
	...
	Hydra::execute_with(|| {
		...
		assert_eq!(charlie_new_hdx_balance_after_first_claim, 1000030740501249);
		...
		assert_eq!(charlie_new_hdx_balance_after_2nd_claim, 1000031130659947);
	});
}
