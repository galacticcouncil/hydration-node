...
use pallet_liquidation::{BorrowerData, BorrowerDataDetails, MAX_LIQUIDATIONS, types::offchain_worker::*};
...
pub fn get_user_account_data(mm_pool: EvmAddress, user: EvmAddress) -> Option<(U256, U256, U256, U256, U256, U256)> {
	let context = CallContext::new_call(mm_pool, user);
...
fn liquidation_should_work() {
	...
	hydra_live_ext(PATH_TO_SNAPSHOT).execute_with(|| {
		...
		let pool_contract = MoneyMarketData::<Block, Runtime>::get_pool(pap_contract, alice_evm_address).unwrap();
...
fn call_methods_for_liquidation_worker() {
	...
	hydra_live_ext(PATH_TO_SNAPSHOT).execute_with(|| {
		...
		// get Pool contract address
		let pool_contract = MoneyMarketData::<Block, Runtime>::get_pool(pap_contract, alice_evm_address).unwrap();
		let oracle_contract = MoneyMarketData::<Block, Runtime>::get_price_oracle(pap_contract, alice_evm_address).unwrap();
		assert_ok!(Liquidation::set_borrowing_contract(
			RuntimeOrigin::root(),
			pool_contract
		));
		...
		println!("USR DATA: \n total coll {:?} \n total debt {:?}\n {:?}\n avg liq threshold {:?}\n LTV {:?}\n HF {:?}", usr_data.0, usr_data.1, usr_data.2, usr_data.3, usr_data.4, usr_data.5);
		...
		let debt_asset = money_market_data.reserves()[4].asset_address();
		let collateral_asset = money_market_data.reserves()[4].asset_address();
		...
		// ensure that the health_factor < 1
		let usr_data = get_user_account_data(pool_contract, alice_evm_address).unwrap();
		assert!(usr_data.5 < U256::from(1_000_000_000_000_000_000u128));
