	...
	loop {
		...
		for &(a, b) in &pairs {
			...
			for c in bc_pairs {
				...
				// Check each intent is satisfiable at spot rate
				let ab_spot = calc_amount_out(ab_best.remaining_in, pa, pb);
				let bc_spot = calc_amount_out(bc_best.remaining_in, pb, pc);
				let ca_spot = calc_amount_out(ca_best.remaining_in, pc, pa);
