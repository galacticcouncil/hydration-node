...
mod tests {
    ...
    fn trade(pool: PoolType<AssetId>, asset_in: AssetId, asset_out: AssetId) -> types::Trade<AssetId> {
        types::Trade { pool, asset_in, asset_out }
    }
    ...
    fn stableswap_then_omnipool() {
        let routes = get_routes(
            1,
            3,
            alloc::vec![stableswap(100, &[1, 2]), omnipool(&[2, 3, 4])],
        );
        assert_eq!(routes.len(), 1);
        assert_eq!(routes[0].len(), 2);
        assert_eq!(routes[0][0].pool, PoolType::Stableswap(100));
    ...
    fn trusted_only_excludes_xyk() {
        let routes = get_routes(
            1,
            3,
            alloc::vec![omnipool(&[1, 2, 3]), xyk(1, 2)],
        );
        assert!(routes
            .iter()
            .all(|r| r.iter().all(|t| t.pool != PoolType::XYK)));
    }
    ...
    fn isolated_only_when_no_trusted_pools_have_assets() {
        let routes = get_routes(
            10,
            30,
            alloc::vec![xyk(10, 20), xyk(20, 30), omnipool(&[1, 2, 3])],
        );
        assert_eq!(routes.len(), 1);
    ...
    fn isolated_only_filters_to_relevant_pools() {
        let routes = get_routes(
            1,
            4,
            alloc::vec![xyk(1, 2), xyk(2, 3), xyk(3, 4)],
        );
        assert!(routes.is_empty());
    }
    ...
    fn exactly_max_trades_succeeds() {
        let pools: Vec<_> = (0u32..9)
            .map(|i| stableswap(i + 100, &[i, i + 1]))
            .collect();
    ...
    mod mainnet {
        ...
        fn usdt_to_usdc_via_stableswap() {
            ...
            dev_msg!(
                "get_routes 10->22: routes={:#?}",
                routes
            );
            assert!(!routes.is_empty());
            assert!(routes.iter().any(|r| r.iter().any(|t| matches!(t.pool, PoolType::Stableswap(_)))));
        }
