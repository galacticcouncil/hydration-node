...
import { seedConcentrated } from './commands/seedConcentrated'
...
program
  ...
  .action(run(seed))
program
  .command('seed-concentrated')
  .description('create + seed a concentrated-liquidity v3 pool (nested narrow positions)')
  .action(run(seedConcentrated))
program
