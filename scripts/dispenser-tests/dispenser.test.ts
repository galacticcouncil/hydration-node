...
async function initializeVaultIfNeeded(api: ApiPromise) {
  ...
  const current = (await (api.query as any).ethDispenser.faucetBalanceWei()).toBigInt()
  const threshold = ((api.consts as any).ethDispenser.minFaucetEthThreshold as any).toBigInt()
  ...
  const setBalCall = (api.tx as any).ethDispenser.setFaucetBalance(addWei.toString())
  await executeAsRootViaScheduler(api, setBalCall, 'Top up ethDispenser faucet balance via Root')
