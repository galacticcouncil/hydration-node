...
fn slip_fee_amount_basic_negative_delta() {
	// Q0 = 10M, prior_delta = 0, delta_q = -99000 (hub asset leaving)
...
fn slip_fee_amount_basic_positive_delta() {
	// Q0 = 5M, prior_delta = 0, delta_q = +97516 (hub asset entering buy pool)
...
fn slip_fee_amount_capped_at_max() {
	...
	let result = calculate_slip_fee_amount(
		1000 * UNIT,
		SignedBalance::zero(),
		SignedBalance::Negative(900 * UNIT),
		max_fee,
		base,
	);
...
fn slip_fee_amount_infeasible_returns_none() {
	...
	let result = calculate_slip_fee_amount(
		1000 * UNIT,
		SignedBalance::zero(),
		SignedBalance::Negative(1000 * UNIT),
		Permill::from_percent(5),
		base,
	);
	...
	let result = calculate_slip_fee_amount(
		1000 * UNIT,
		SignedBalance::zero(),
		SignedBalance::Negative(1500 * UNIT),
		Permill::from_percent(5),
		base,
	);
	assert_eq!(result, None);
...
fn sell_with_slip_spec_example() {
	...
	// HDX: Q₀ = 10M hub asset, R = 10M HDX
	// DOT: Q₀ = 5M hub asset, R = 500K DOT
	// s = 1, max_slip_fee = 5%
	...
	// SELL SIDE:
	// delta_qi = 10M * 100K / (10M + 100K) = 10^12 / 10.1M ≈ 99009.9009... hub asset
	let delta_hub_in = *result.asset_in.delta_hub_reserve;
	// This is ~99009.9 hub asset (in 12 decimal units)
	assert!(delta_hub_in > 99_009 * UNIT && delta_hub_in < 99_010 * UNIT);

	// Slip sell rate ≈ 0.999% (from spec)
	// Protocol fee = 0.05% of delta_hub_in
	// D_gross ≈ 97,516 hub asset (from spec: 99000 * (1 - 0.05% - 0.999%))
	// Slip buy rate ≈ 1.91%
	// D_net ≈ 95,654 hub asset (from spec: 97516 * (1 - 1.91%))
	let d_net = *result.asset_out.delta_hub_reserve;
	// D_net should be roughly 95,654 * UNIT
	// Allow some tolerance for rounding differences
	assert!(
		d_net > 95_000 * UNIT && d_net < 96_500 * UNIT,
		"D_net should be ~95,654 hub asset, got {}",
		d_net / UNIT
	);
...
fn sell_hub_buy_hub_roundtrip_with_slip() {
	...
	// Sell X hub asset → get Y tokens
	let sell_result = calculate_sell_hub_state_changes(&asset_out_state, hub_amount, asset_fee, Some(&slip)).unwrap();

	let tokens_received = *sell_result.asset.delta_reserve;

	// Buy Y tokens → costs how much hub asset?
	let buy_result =
...
fn cross_validate_scenario1_sell_hdx_for_dot() {
	...
	// Pool: HDX(10M liquidity, 10M hub asset), DOT(500K liquidity, 5M hub asset)
	// hub_fee=0.0005, asset_fee=0.0025, slip_factor=1.0
	let asset_in_state = AssetReserveState {
		reserve: 10_000_000 * UNIT,
		hub_reserve: 10_000_000 * UNIT,
		shares: 10_000_000 * UNIT,
...
#[test]
fn cross_validate_scenario2_sell_hub_for_dot() {
	// Python scenario 2: Sell 10K hub asset → DOT (hub trade, buy-side slip only)
	// Pool: DOT(500K liquidity, 5M hub asset), asset_fee=0.0025, slip_factor=1.0
	let asset_out_state = AssetReserveState {
		reserve: 500_000 * UNIT,
		hub_reserve: 5_000_000 * UNIT,
		shares: 500_000 * UNIT,
...
fn cross_validate_scenario3_buy_dot_with_hdx() {
	...
	// Pool: HDX(10M/10M), DOT(500K/5M), hub_fee=0.0005, asset_fee=0.0025, slip_factor=1.0
	let asset_in_state = AssetReserveState {
		reserve: 10_000_000 * UNIT,
		hub_reserve: 10_000_000 * UNIT,
		shares: 10_000_000 * UNIT,
...
fn cross_validate_scenario4_consecutive_sells() {
	...
	let hdx_delta = SignedBalance::Negative(49_751_243_781_094_527); // hub asset left HDX pool
	let dot_delta = SignedBalance::Positive(49_115_249_061_335_511); // hub asset entered DOT pool
...
fn cross_validate_scenario5_opposing_flow() {
	// Trade 1: Sell 50K HDX → DOT (HDX loses hub asset, DOT gains hub asset)
	// Trade 2: Sell 5K DOT → HDX (opposing direction: reverses the deltas partially)
	//
	// For trade 2, DOT is sell pool with positive prior delta (hub asset entered before),
	// HDX is buy pool with negative prior delta (hub asset left before).
	...
	// Python-computed deltas after trade 1
	let hdx_delta = SignedBalance::Negative(49_751_243_781_094_527); // hub asset left HDX pool (negative)
	let dot_delta = SignedBalance::Positive(49_115_249_061_335_511); // hub asset entered DOT pool (positive)
	...
	let slip2 = TradeSlipFees {
		...
		asset_in_delta: dot_delta,                // positive (hub asset entered)
		asset_out_hub_reserve: 10_000_000 * UNIT, // HDX Q0
		asset_out_delta: hdx_delta,               // negative (hub asset left)
		max_slip_fee: Permill::from_percent(100),
	};
...
fn cross_validate_scenario6_buy_hub_with_prior_delta() {
	...
	// Trade 2: Buy 100 DOT for hub asset (hub trade, with prior delta on buy pool)
	//
	// Using Python-computed intermediate state to isolate trade 2 cross-validation.

	// Python-computed DOT delta and pool state after trade 1 (20K HDX→DOT, hub_fee=0)
	let dot_delta = SignedBalance::Positive(19_890_712_487_233_615); // positive (hub asset entered DOT pool)
	...
	// Trade 2: Buy 100 DOT for hub asset (hub trade with prior positive delta)
	...
	// Buy with prior positive delta should cost MORE hub asset than fresh block
...
fn cross_validate_scenario9_buy_opposing_flow() {
	...
	// Trade 1: Sell 50K HDX → DOT (HDX loses hub asset, DOT gains hub asset)
	// Trade 2: Buy 5000 HDX with DOT (opposing: DOT sells hub asset out, HDX receives hub asset)
	//
	// Using Python-computed intermediate state to isolate trade 2.
	...
	// Python-computed deltas after trade 1
	// DOT (now sell pool) had positive delta (hub asset entered)
	// HDX (now buy pool) had negative delta (hub asset left)
	...
	let slip_same_dir = TradeSlipFees {
		...
		asset_in_delta: dot_delta.negate(), // flip to same direction (negative = hub asset left)
		asset_out_hub_reserve: 10_000_000 * UNIT,
		asset_out_delta: hdx_delta.negate(), // flip to same direction (positive = hub asset entered)
		max_slip_fee: Permill::from_percent(100),
	};
...
struct PoolAsset {
	reserve: Balance,
	hub_reserve: Balance,
	q0: Balance,          // hub asset at block start (fixed for the block)
	delta: SignedBalance, // cumulative intra-block hub asset delta
	shares: Balance,
}
...
/// Execute a sell-hub-asset-for-token trade and update pool state + delta.
/// Returns (tokens_out, slip_fee).
fn exec_sell_hub(buy_asset: &mut PoolAsset, hub_amount: Balance, asset_fee: Permill) -> (Balance, Balance) {
	let slip = HubTradeSlipFees {
...
fn run_order_a() -> SequenceResult {
	...
	// Trade 6: Sell 5K hub asset → ETH (hub trade, buy-side only)
	...
	// Trade 10: Sell 5K DOT → HDX (opposing on HDX which lost a lot of hub asset)
...
fn run_order_b() -> SequenceResult {
	...
	// Trade 4: Sell 5K hub asset → ETH (was trade 6)
	let (out, fee) = exec_sell_hub(&mut eth, 5_000 * UNIT, AF);
	fees.push(fee);
	outputs.push(out);
...
fn multi_trade_sequence_order_a() {
	...
	// (hub_reserve = starting_hub_reserve + net hub asset movement, and delta tracks that net movement)
	let check_hub_delta = |name: &str, hub: Balance, q0: Balance, delta: SignedBalance| {
		...
		// The hub_reserve tracks actual pool state; delta tracks hub asset movement at the pool level.
...
// Python values from test_slip_fee_cross_validation.py (hub_mint_pct=0, slip_factor=1.0).
...
fn multi_trade_cross_validation_order_a() {
	...
	let py_outputs: [u128; 12] = [
		...
		124_857_762_387_519,    // trade 6: sell 5K hub asset→ETH
...
fn multi_trade_cross_validation_order_b() {
	...
	let py_outputs: [u128; 12] = [
		...
		125_308_889_078_455,    // trade 4: sell 5K hub asset→ETH
...
fn assert_buy_round_trip(
	...
	let state_changes =
		calculate_buy_state_changes(in_state, out_state, amount, asset_fee, protocol_fee, burn_fee, slip)
			.expect("calculate_buy_state_changes should succeed");

	let delta_hub_in = match state_changes.asset_in.delta_hub_reserve {
		BalanceUpdate::Decrease(v) => v,
		_ => panic!("expected Decrease for asset_in hub reserve"),
	};
	let total_pf = state_changes.fee.protocol_fee;

	let d_net_expected = buy_d_net(out_state, amount, asset_fee);
	let d_net_actual = delta_hub_in
		.checked_sub(total_pf)
		.expect("delta_hub_in must be >= total_pf");

	assert!(
		d_net_actual >= d_net_expected,
		"pool UNDERFUNDED: hub_in={delta_hub_in} total_pf={total_pf} d_net_actual={d_net_actual} < d_net_target={d_net_expected}",
	);
	let overshoot = d_net_actual - d_net_expected;
	assert!(
		overshoot <= MAX_OVERSHOOT,
		"over-charged by {overshoot} (limit {MAX_OVERSHOOT}): hub_in={delta_hub_in} total_pf={total_pf} d_net_actual={d_net_actual} d_net_target={d_net_expected}",
	);
}
...
fn calculate_buy_should_round_trip_when_no_slip_cap_fires() {
	...
	let slip = TradeSlipFees {
		...
		asset_out_hub_reserve: asset_out_state.hub_reserve,
		asset_out_delta: SignedBalance::zero(),
		max_slip_fee: Permill::from_percent(1),
	};
...
fn calculate_buy_should_round_trip_tightly_when_caps_fire_on_both_sides() {
	...
	// And specifically check the cap-cap regime is exactly tight (zero overshoot)
	// because both capped formulas are closed-form inverses.
	let state_changes = calculate_buy_state_changes(
		&asset_in_state,
		&asset_out_state,
		amount,
	...
	let total_pf = state_changes.fee.protocol_fee;
	let d_net_actual = delta_hub_in - total_pf;
	let d_net_expected = buy_d_net(&asset_out_state, amount, Permill::zero());
	assert_eq!(
		d_net_actual, d_net_expected,
		"cap-cap regime should be tight (closed-form inverse on both sides)",
	);
}
...
fn calculate_buy_should_round_trip_when_cap_fires_on_buy_side_with_protocol_fee() {
	...
	assert_buy_round_trip(
		...
		20_000 * UNIT,
		Permill::zero(),
		Permill::from_rational(1u32, 1000u32), // 0.1% protocol fee
		Permill::zero(),
		Some(&slip),
	);
}
...
fn calculate_buy_should_round_trip_when_both_caps_fire_with_all_fees() {
	...
	assert_buy_round_trip(
		...
		&asset_out_state,
		20_000 * UNIT,
		Permill::from_rational(25u32, 10_000u32), // 0.25% asset fee
		Permill::from_rational(1u32, 1000u32),    // 0.1% protocol fee
		Permill::from_percent(20),                // 20% burn fee
		Some(&slip),
	);
}
...
#[test]
fn calculate_buy_should_round_trip_when_caps_fire_with_negative_input_delta() {
	let asset_in_state = AssetReserveState {
		reserve: 1_000_000 * UNIT,
		hub_reserve: 1_000_000 * UNIT,
		shares: 1_000_000 * UNIT,
	...
	let slip = TradeSlipFees {
		asset_in_hub_reserve: asset_in_state.hub_reserve,
		asset_in_delta: SignedBalance::Negative(5_000 * UNIT),
		asset_out_hub_reserve: asset_out_state.hub_reserve,
		asset_out_delta: SignedBalance::zero(),
		max_slip_fee: Permill::from_percent(1),
	};
...
#[test]
fn calculate_buy_should_round_trip_when_caps_fire_with_positive_output_delta() {
	let asset_in_state = AssetReserveState {
		reserve: 1_000_000 * UNIT,
		hub_reserve: 1_000_000 * UNIT,
		shares: 1_000_000 * UNIT,
	...
	let slip = TradeSlipFees {
		...
		asset_in_delta: SignedBalance::zero(),
		asset_out_hub_reserve: asset_out_state.hub_reserve,
		asset_out_delta: SignedBalance::Positive(5_000 * UNIT),
		max_slip_fee: Permill::from_percent(1),
	};
	assert_buy_round_trip(
		&asset_in_state,
...
fn calculate_buy_should_round_trip_when_rate_is_just_below_cap_threshold() {
	...
	assert_buy_round_trip(
		&asset_in_state,
		&asset_out_state,
		500_000 * UNIT, // ~0.5% hub flow, under 1% cap
		Permill::zero(),
		Permill::zero(),
		Permill::zero(),
		Some(&slip),
...
fn calculate_buy_for_hub_asset_should_round_trip_when_slip_cap_fires() {
	...
	let d_net = d_gross.checked_sub(slip_fee).expect("d_gross must be >= slip fee");
	let d_net_target = buy_d_net(&asset_out_state, amount, Permill::zero());

	assert!(d_net >= d_net_target, "pool underfunded: {d_net} < {d_net_target}");
	let overshoot = d_net - d_net_target;
...
fn calculate_buy_should_round_trip_when_trade_is_close_to_pool_limit() {
	...
	assert_buy_round_trip(
		&asset_in_state,
		&asset_out_state,
		100_000 * UNIT, // 10% of pool — deeply in capped regime
		Permill::zero(),
		Permill::zero(),
		Permill::zero(),
		Some(&slip),
...
fn calculate_buy_should_round_trip_across_parameter_sweep() {
	...
	let max_slips = [
		...
		Permill::from_percent(5),
		Permill::from_rational(1u32, 1000u32), // 0.1%
	];
	let pfs = [Permill::zero(), Permill::from_rational(1u32, 1000u32)];
	let asset_fees = [Permill::zero(), Permill::from_rational(25u32, 10_000u32)];
	let prior_deltas = [
		SignedBalance::zero(),
		SignedBalance::Positive(2_000 * UNIT),
		SignedBalance::Negative(2_000 * UNIT),
	];

	for &in_sz in &pool_sizes {
		for &out_sz in &pool_sizes {
			for &amt_ratio in &amount_ratios {
				...
				for &max_slip in &max_slips {
					for &pf in &pfs {
						for &asset_fee in &asset_fees {
							for &in_delta in &prior_deltas {
								for &out_delta in &prior_deltas {
									let in_state = AssetReserveState {
										reserve: in_sz * UNIT,
										hub_reserve: in_sz * UNIT,
										shares: in_sz * UNIT,
										protocol_shares: 0u128,
									};
									let out_state = AssetReserveState {
										reserve: out_sz * UNIT,
										hub_reserve: out_sz * UNIT,
										shares: out_sz * UNIT,
										protocol_shares: 0u128,
									};
									let slip = TradeSlipFees {
										asset_in_hub_reserve: in_state.hub_reserve,
										asset_in_delta: in_delta,
										asset_out_hub_reserve: out_state.hub_reserve,
										asset_out_delta: out_delta,
										max_slip_fee: max_slip,
									};
									assert_buy_round_trip(
										&in_state,
										&out_state,
...
#[test]
fn calculate_slip_fee_amount_should_return_max_when_rate_exceeds_cap() {
	// cum / denom = 5_000_000 / (10_000_000 + 5_000_000) = 1/3 ≈ 33% > 1% cap.
	let hub_reserve = 10_000_000 * UNIT;
	let cum = SignedBalance::Positive(5_000_000 * UNIT);
	let base = 1_000_000 * UNIT;
	let max = Permill::from_percent(1);

	let slip = calculate_slip_fee_amount(hub_reserve, SignedBalance::zero(), cum, max, base).unwrap();
	let expected = max.mul_floor(base);
	assert_eq!(slip, expected, "cap must bind exactly: slip={slip} expected={expected}");
}
...
fn calculate_slip_fee_amount_should_return_proportional_when_rate_below_cap() {
	...
	let slip = calculate_slip_fee_amount(hub_reserve, SignedBalance::zero(), cum, max, base).unwrap();
	let capped_value = max.mul_floor(base);
	assert!(
		slip < capped_value,
		"cap must NOT bind here: slip={slip} cap_value={capped_value}",
	);
}
...
#[test]
fn invert_buy_side_slip_should_use_uncapped_path_when_below_cap() {
	use crate::omnipool::slip_fee::invert_buy_side_slip;
...
#[test]
fn invert_sell_side_fees_should_round_trip_in_capped_regime() {
	use crate::omnipool::slip_fee::invert_sell_side_fees;
...
fn calculate_buy_state_changes_should_charge_capped_slip_on_both_sides_when_both_caps_bind() {
	...
	let u = match r.asset_in.delta_hub_reserve {
		BalanceUpdate::Decrease(v) => v,
