...
parameter_types! {
	...
	pub const HostStateMachine: StateMachine = StateMachine::Polkadot(2034); // polkadot
	pub const StableCoin: AssetId = CORE_ASSET_ID; // FIXME: this should be a stablecoin, not the core asset
}

impl pallet_ismp::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	// Modify the consensus client's permissions, for example, TechAdmin
	type AdminOrigin = TechCommitteeMajority;
