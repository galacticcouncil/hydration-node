...
pub fn set_ed(asset_id: AssetId, ed: u128) -> Result<(), ()> {
	with_transaction(|| {
		TransactionOutcome::Commit(AssetRegistry::update(
			RawOrigin::Root.into(),
			asset_id,
...
pub fn get_contract_bytecode() -> Vec<u8> {
...
pub fn deploy_contract(deployer: EvmAddress) -> EvmAddress {
	deploy_contract_code(get_contract_bytecode(), deployer)
}
