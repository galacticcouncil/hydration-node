...
use pallet_evm::{ExitError, ExitFatal, ExitRevert, ExitSucceed, Log};
use primitives::{AssetId, Balance /*DexShare*/};
use scale_info::TypeInfo;
use sp_core::{H160, U256};
use sp_runtime::{traits::Convert, DispatchError};
use sp_std::prelude::*;
use sp_std::{marker::PhantomData, result::Result, vec::Vec};
//TODO: copied from primitives
...
pub trait AddressMappingT<AccountId> {
...
pub trait AssetIdMappingT<ForeignAssetId, MultiLocation, AssetMetadata> {
...
pub trait Erc20InfoMappingT {
	...
	fn name(currency_id: AssetId) -> Option<Vec<u8>>;
	/// Returns the symbol associated with a given CurrencyId.
	/// If CurrencyId is CurrencyId::DexShare and contain DexShare::Erc20,
	/// the EvmAddress must have been mapped.
	fn symbol(currency_id: AssetId) -> Option<Vec<u8>>;
	/// Returns the decimals associated with a given CurrencyId.
	/// If CurrencyId is CurrencyId::DexShare and contain DexShare::Erc20,
	/// the EvmAddress must have been mapped.
	fn decimals(currency_id: AssetId) -> Option<u8>;
...
impl Erc20InfoMappingT for () {
	fn name(_currency_id: AssetId) -> Option<Vec<u8>> {
		None
	}

	fn symbol(_currency_id: AssetId) -> Option<Vec<u8>> {
		None
	}

	fn decimals(_currency_id: AssetId) -> Option<u8> {
		None
	}

	fn encode_evm_address(_v: AssetId) -> Option<EvmAddress> {
		None
	}

	fn decode_evm_address(_v: EvmAddress) -> Option<AssetId> {
		None
	}
}
...
pub trait InputT {
	...
	fn currency_id_at(&self, index: usize) -> Result<AssetId, Self::Error>;
