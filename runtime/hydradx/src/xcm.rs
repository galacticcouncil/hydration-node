...
pub struct OmniExchanger;

impl AssetExchange for OmniExchanger {
	fn exchange_asset(
		...
		if maximal {
			...
			let account = if origin.is_none() {
				[9; 32] // fake generated account for now
			} else {
				return Err(give);
			};
			let origin = RuntimeOrigin::signed(account.into());
			if give.len() != 1 {
				return Err(give);
...
mod tests {
	...
	fn omni_exchanger_exchanges_supported_assets() {
		let give = MultiAsset::from((GeneralIndex(0), 50 * UNITS)).into();
		let want = MultiAsset::from((GeneralIndex(1), 100 * UNITS)).into();
		OmniExchanger::exchange_asset(None, give, &want, SELL);
	}
}
