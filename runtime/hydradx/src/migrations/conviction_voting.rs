...
use super::*;
use crate::migrations::scheduler::MigrateSchedulerTo6sBlocks;
...
fn calculate_new_block<T: pallet::Config>(
			...
			match &mut voting {
				Voting::Casting(casting) => {
					...
					if unlock_block > current_block {
						casting.prior.0 = calculate_new_block::<T>(current_block, unlock_block);
				...
				Voting::Delegating(delegating) => {
					...
					if unlock_block > current_block {
						delegating.prior.0 = calculate_new_block::<T>(current_block, unlock_block);
						write_to_storage = true;
					};
				}
			};
