...
fn allowed_or_recurse<RuntimeCall>(inst: &Instruction<RuntimeCall>) -> Either<bool, &Xcm<()>> {
	match inst {
		...
		DepositAsset { .. } |
		SetTopic(..) | ClearTopic |
		ExpectAsset(..) | BurnAsset(..) |
		BuyExecution  { .. } => Either::Left(true),
		InitiateReserveWithdraw { xcm, .. } | DepositReserveAsset { xcm, .. } | TransferReserveAsset { xcm, .. } => Either::Right(xcm),
		_ => Either::Left(false),
	}
}

fn check_instructions_recursively<RuntimeCall>(xcm: &Xcm<RuntimeCall>, depth: u16) -> bool {
	if depth > 6 { return false } // TODO: make configurable?
	...
	while let (true, Some(inst)) = (count.get() < limit, iter.next()) {
		...
		match allowed_or_recurse(inst) {
			...
			Either::Right(xcm) => if check_instructions_recursively(xcm, depth + 1) {
				continue
			} else {
				return false
			}
...
mod tests {
	...
	fn allow_transfer_and_swap_should_filter_transact() {
		...
		let xcm = Xcm(vec![Transact { origin_kind: OriginKind::Native, require_weight_at_most: Weight::from_parts(1, 1), call: call.into() }]);
		let loc = MultiLocation::new(0, AccountId32 {
			network: None,
			id: [0; 32].into()
		});
		assert_eq!(AllowTransferAndSwap::<crate::RuntimeCall>::contains(&(loc, xcm)),
		false);
	}
}